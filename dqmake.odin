...
main :: proc() {
	...
	mapData.fullpath = fmt.aprint(args={loadpath, filepath.SEPARATOR_STRING, "maps", filepath.SEPARATOR_STRING, "new_dqmake_map.dqm"}, sep="")
	...
	for !rl.WindowShouldClose() {
		...
		_draw :: proc() {
			...
			gui.drawText({10, 40}, 25, gui.ACTIVE_COLOR, fmt.tprint("tile :       ", rune(tileSelected), tileSelected))
			...
			switch menuKind {
				...
				case .FILE:
					if fileMenuElemsCount > 0 {
						...
						if fileMenuButtonBool {
							...
							ok : bool
							tiles.loadFromFile(fileMenuElems[gui.menuContext.selected].(gui.menuFileButton_t).fullpath, &mapData)
							menuKind = .NONE
						}
				...
				case .TILE:
					...
					if tileMenuButtonBool {
						...
						tileSelected = tiles.kind_t(tileMenuElems[gui.menuContext.selected].(gui.menuButton_t).name[0]) // ughh
						menuKind = .NONE
					}
			}
		} _draw()
...
isTilePosValid :: proc(pos : ivec2) -> bool {
	return pos.x>=0 && pos.y>=0 && pos.x<tiles.MAP_MAX_SIZE && pos.y < tiles.MAP_MAX_SIZE && pos.x<mapData.bounds.x && pos.y<mapData.bounds.y
}
	...
	mapSelectFilesFetchDirAndAppend :: proc(dir : string) {
		...
		for i := 0; i < len(filebuf); i += 1 {
			...
			fileMenuElems[fileMenuElemsCount] = gui.menuFileButton_t{
				fileinfo.name[:dotindex], fileinfo.fullpath, &fileMenuButtonBool,
			}
