			...
			for y : u32 = 0; y < tilemap.bounds[1]; y += 1 {
				...
				#partial switch tilemap.tiles[x][y] {
					...
					case map_tileKind_t.EMPTY:
						...
						rl.DrawCube(pos0, size.x, size.y, size.z, rl.RED)
						rl.DrawCube(pos1, size.x, size.y, size.z, rl.GREEN)
				}
			}
		}
	}
...
map_toTilePos :: proc(p : vec3) -> ivec2 {
...
map_clearAll :: proc() {
	...
	for x : u32 = 0; x < TILEMAP_MAX_WIDTH; x += 1 {
		for y : u32 = 0; y < TILEMAP_MAX_WIDTH; y += 1 {
			tilemap.tiles[x][y] = map_tileKind_t.WALL
		}
	}
}
...
map_loadFromFile :: proc(name: string) {
	...
	println("map_loadFromFile fullpath", fullpath)
	...
	dataloop : for index < cast(i32)len(data) {
		...
		tilemap.tiles[x][y] = cast(map_tileKind_t)ch
		x += 1
...
phy_getTileBox :: proc(coord : ivec2, pos : vec3) -> (phy_box_t, bool) {
	...
	#partial switch tileKind {
		...
		case map_tileKind_t.EMPTY: 
			//box : vec3 = pos.y > 0.0 ?
			//	phy_box_t{vec3{posxz[0],-TILE_HEIGHT+TILE_MIN_HEIGHT, posxz[1]}, vec3{TILE_SIZE, TILE_MIN_HEIGHT, TILE_SIZE}/2}
			//	:
			//	phy_box_t{vec3{posxz[0], TILE_HEIGHT-TILE_MIN_HEIGHT, posxz[1]}, vec3{TILE_SIZE, TILE_MIN_HEIGHT, TILE_SIZE}/2}
	}
...
phy_clipSphereWithBox :: proc(pos : vec3, rad : f32, boxSize : vec3) -> (vec3, vec3, f32, bool) {
	...
	sd := linalg.vector_length(outdir) + min(max(q.x, max(q.y, q.z)), 0.0)
	
	if sd < rad {
		outdir /= sd == 0.0 ? 1.0 : sd
		outdir *= possign
		norm := isCenterInside ? indir : outdir
		offs := isCenterInside ? indir*(boxSize + vec3{rad,rad,rad}*0.5) : outdir*(rad-sd)*rad
		...
		for z : i32 = 0; z <= 1; z += 1 {
			...
			if isfull {
				...
				if boxhit {
					...
					//println("res_offs", res_offs, "offs", offs)
				}
				hit |= boxhit
...
player_update :: proc() {
	...
	player_velocity.y -= PLAYER_GRAVITY * deltatime * (player_isGrounded ? 0.1 : 1.0)
	...
	player_pos = player_pos + phy_vec
	
	if phy_hit && phy_norm.y > PLAYER_MIN_NORMAL_Y do player_isGrounded = true
