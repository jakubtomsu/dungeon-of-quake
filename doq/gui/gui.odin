...
updateAndDrawElemBuf :: proc(elems : []menuElem_t) -> bool {
	...
	for i := 0; i < len(elems); i += 1 {
		...
		switch elem in elems[i] {
			...
			case menuFileButton_t:
				drawText({nameoffs, offs}, SIZE, vcol, elem.name)
				if isSelected {
					drawText({valoffs, offs+SIZE*0.25}, SIZE*0.5, {col.r,col.g,col.b,100}, elem.fullpath)
				}
		}
	...
	if selectedOffs > f32(menuContext.windowSizeY)-SCROLL_MARGIN { // bottom
		menuContext.startOffs += (f32(menuContext.windowSizeY)-SCROLL_MARGIN - selectedOffs) * clamp(menuContext.deltatime * SCROLL_SPEED, 0.0, 1.0)
	}

	if selectedOffs < SCROLL_MARGIN { // top
		menuContext.startOffs += (SCROLL_MARGIN - selectedOffs) * clamp(menuContext.deltatime * SCROLL_SPEED, 0.0, 1.0)
	}
