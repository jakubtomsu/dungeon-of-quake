...
// @returns: tn, hitenemy
bullet_shootRaycast :: proc(start : vec3, dir : vec3, damage : f32, rad : f32, col : vec4, effectDuration : f32) -> (tn :f32, enemykind : enemy_kind_t, enemyindex : i32) {
	hit : bool
...
_enemy_updateDataAndRender :: proc() {
	...
	if !gameIsPaused {
		...
		for i : i32 = 0; i < enemy_data.knightCount; i += 1 {
			...
			if enemy_data.knights[i].isMoving && speed < ENEMY_KNIGHT_MAX_SPEED && isOnGround && enemy_data.grunts[i].animState!=.ATTACK {
	...
	for i : i32 = 0; i < enemy_data.gruntCount; i += 1 {
		...
		// anim
		{
			...
			rl.UpdateModelAnimation(asset_data.enemy.gruntModel, asset_data.enemy.gruntAnim[animindex], enemy_data.grunts[i].animFrame)
		}

		rl.DrawModelEx(
			asset_data.enemy.gruntModel,
			enemy_data.grunts[i].pos,
			{0,1,0}, enemy_data.grunts[i].rot*180.0/math.PI,
			1.4,
			rl.WHITE,
		)
	}
